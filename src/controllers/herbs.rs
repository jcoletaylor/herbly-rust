...
pub async fn get_one(req: tide::Request<state::State>) -> tide::Result {
    ...
    let herb = handlers::herbs::get_one(format!("{}", name), &db_pool).await?;
    let mut res = Response::new(200);
    res.set_body(Body::from_json(&herb)?);
    Ok(res)
}
...
pub async fn get_all(req: tide::Request<state::State>) -> tide::Result {
    ...
    let herbs = handlers::herbs::get_all(limit, offset, &db_pool).await?;

    let mut res = Response::new(200);
    res.set_body(Body::from_json(&herbs)?);
    Ok(res)
}
